...
public class Main {
    ...
    public String sendMessage(String toAccountID, String subject, String body) throws IOException {
        requireLogin();
        var encodedSubject = Base64.getEncoder().encodeToString(subject.getBytes("UTF-8"));
        var encodedBody = Base64.getEncoder().encodeToString(body.getBytes("UTF-8"));
     ...
     */
    public String requestEndpoint(String endpoint, Map<String, String> params) throws IOException {
        requireLogin();
        if (params == null) params = new HashMap<>();
        params.put("accountID", this.accountID);
        params.put("gjp2", this.gjp2);
        params.put("secret", "Wmfd2893gb7");
        return EndpointRequest.doPost("/database/" + endpoint, params, null);
    }
}
